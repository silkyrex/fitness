...
This repo is a personal fitness data hub. The only runnable code is `scripts/oura_sync.py`, which pulls Oura API v2 data and upserts it into a Notion "Daily Log" database. There is no server, database, or build step. Sole Python dependency is `requests` (installed by the update script). See `README.md` for schemas, workflows, and CLI flags.
